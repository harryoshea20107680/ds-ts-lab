...
//Returns all the Friends in a buddy list
function getBuddyListFriends(list: BuddyList): Friend[] {
    const buddyListFriends : Friend[] =[];
    for (const buddyListMember of list.members) {
        if(!('department' in buddyListMember)){
            buddyListFriends.push(buddyListMember as Friend)
        }
    }
    return buddyListFriends
}

console.log('Friends in selected buddy list: ' , getBuddyListFriends(myFootballBuddies).map(friend => friend.name))
